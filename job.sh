...
#module swap mpich2-x86_64  openmpi-x86_64

#umask 002
DATA_LOCATION=/mnt/locutus/remotesensing/6ru/k_clustering/data
mkdir -p results 
mkdir -p logfiles
mkdir -p profiles
CASE=ro_0-99.smooth_2017-2020
...
RAND_SEEDS=/home/jbk/bin/rand_seeds.${MACHINE}
PRAND_SAMPLE=/home/jbk/bin/prand_sample_bj.${MACHINE}
PSAMPLE=/home/jbk/bin/psample_dataset_new2.${MACHINE}
SEED_BINARY=/home/jbk/bin/pcluster_Ndist_accel.${MACHINE}
CLUSTER_PROCS=16
CLUSTER_BINARY=~jbk/bin/pcluster_Ndist_accel.${MACHINE}
...
if [ ! -f obs.bin.${CASE} ]; then
   ...
   time ${STANDARDIZE} -b -v -r ${ROWS} -c ${COLS} -m mean.${CASE}.orig -s stddev.${CASE}.orig -o obs.bin.${CASE} ${DATA_LOCATION}/obs.raw.${CASE}
fi
###############################################################################
...
if [ ! -f seeds.out.${CASE}.${NUMCLUST} ]; then
   ...
   for (( t=0 ; t < ${NUMTRIALS}; t++ )) ; do
      if [ ! -f cluster.stats.${CASE}_sample${t}.${NUMCLUST} ]; then
         ...
         set -x
         time mpirun -n ${SEED_PROCS} ${PSAMPLE} --rand_sampler -r ${ROWS} -c ${COLS} -k ${SAMPLE} -x ${NUMPARTS} -o obs.bin.${CASE}_sample${t} obs.bin.${CASE}
         set +x
         echo "Generating random seeds for trial ${t}"
         time ${RAND_SEEDS} -k ${NUMCLUST} -r ${SAMPLE} -c ${COLS} -o seeds.out.${CASE}_sample${t}.${NUMCLUST} obs.bin.${CASE}_sample${t}
         ...
         time mpirun -n ${SEED_PROCS} ${SEED_BINARY} -z clust.res -p seeds.res -k ${NUMCLUST} -r ${SAMPLE} -c ${COLS} -b ${SUB_NALIQUOT} -i seeds.out.${CASE}_sample${t}.${NUMCLUST} -o seeds.out.${CASE}_sample${t}.${NUMCLUST}.final -a clusters.out.${CASE}_sample${t}.${NUMCLUST} obs.bin.${CASE}_sample${t} >& ${LOGFILE}
   ...
   time ${STANDARDIZE} -n -b -v -r ${P2SAMPLE} -c ${COLS} -m mean.${CASE}_phase2.orig -s stddev.${CASE}_phase2.orig -o obs.bin.${CASE}_phase2 obs.raw.${CASE}_phase2
   for (( t=0 ; t < ${NUMTRIALS}; t++ )) ; do
      ...
      time mpirun -n ${SEED_PROCS_P2} ${SEED_BINARY} -z clust.res -p seeds.res -k ${NUMCLUST} -r ${P2SAMPLE} -c ${COLS} -b ${SUB_NALIQUOT} -i seeds.out.${CASE}_sample${t}.${NUMCLUST}.final -o seeds.out.${CASE}_phase2_sample${t}.${NUMCLUST}.final -a clusters.out.${CASE}_phase2_sample${t}.${NUMCLUST} obs.bin.${CASE}_phase2 >& ${LOGFILE}
   ...
   for (( t=0 ; t < ${NUMTRIALS}; t++ )) ; do
      echo "${t} `grep 'Mean centroid to obs distance:' cluster.stats.${CASE}_phase2_sample${t}.${NUMCLUST} | awk '{printf "%s\n", $NF;}'`" >> stats.${CASE}_phase2
   done
...
# Penguins
time mpirun -n ${CLUSTER_PROCS} ${CLUSTER_BINARY} -z clust.res -p seeds.res -k ${NUMCLUST} -r ${ROWS} -c ${COLS} -b ${NALIQUOT} -i seeds.out.${CASE}.${NUMCLUST} -o results/seeds.out.${CASE}.${NUMCLUST}.final -a results/clusters.out.${CASE}.${NUMCLUST} obs.bin.${CASE} >& ${LOGFILE}
...
# Penguins
time ${STANDARDIZE} -u -v -r ${NUMCLUST} -c ${COLS} -M mean.${CASE}.orig -S stddev.${CASE}.orig -o results/seeds.out.${CASE}.${NUMCLUST}.final.unstd results/seeds.out.${CASE}.${NUMCLUST}.final
echo "`date` -- Done"
